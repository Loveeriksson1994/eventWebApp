...
function getCategory(node) {
  ...
  categoryList.forEach((item) => {
    if (categoryList.indexOf(item) != categoryList.length - 1) {
...
function eventIsActive(node) {
  ...
  if (endDateForSort >= currentDateFormated) {
    return true;
  } else {
    return false;
  }
...
function byDate(a, b) {
  return (
    new Date(a.startDateForSort).valueOf() -
    new Date(b.startDateForSort).valueOf()
  );
}
